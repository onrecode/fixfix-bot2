...
1. **Добавить секреты в GitHub**:
   - `PROD_HOST` - IP адрес сервера
   - `PROD_USERNAME` - имя пользователя для SSH
   - `PROD_SSH_KEY` - приватный SSH ключ
2. **Протестировать деплой** - сделать push в main ветку
3. **Проверить работу сервисов** на сервере
4. **Убедиться в доступности** API и бота

**Текущий статус**: Workflow готов, деплой включен, нужны только секреты
